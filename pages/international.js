...
class International extends React.Component {
  anchorLinks = [
    {
      href: "#reviews-country-wo-france",
      text: "Répartition Avis",
      name: "reviews"
    },
    {
      href: "#monthly-evolution-reviews",
      text: "Evolution Mensuelle",
      name: "reviews-monthly"
    },
    {
      href: "#yearly-evolution-reviews-wo-france",
  ...
  static async getInitialProps({ req, store, isServer, pathname, query }) {
    ...
    const response = await axios.get(
      `${state.api}/${state.study}/international/${year}/annual`
    );
  ...
  async componentDidMount() {
    try {
      const monthRes = await this.axiosProgress(
        `${this.props.api}/${this.props.study}/international/${
          this.props.year
        ...
        <div className="options-menu">
          ...
          >
            <form onSubmit={this.handleSubmit.bind(this)}>
              ...
              <div className="form-group row">
                <label className="col-md-1 col-form-label text-muted  ml-auto">
                  Ages
                </label>
                <MultiSelect
                  class="col-md-2"
        ...
        <div className="col">
          <div className="test">
            ...
            <div className="row stats">
              ...
              />
              <Stat
                value={this.state.maxEvolution.label}
                type={this.props.t("stats:present")}
                background={statsColors["central"]}
                fa="fas fa-map-pin"
                addValue={this.state.maxEvolution.value["diff"].value}
              />
              <Stat
                value={this.state.data["TotalReviews"][
                  this.state.selectedYear["value"]
                ].NB1.toLocaleString()}
                type={this.props.t("stats:reviews")}
                background={statsColors["reviews"]}
            ...
            <div className="row">
              <DataViz
                anchorId="reviews-country-wo-france"
                title={this.props.t("reviews-wo-france")}
                style={{ borderLeft: statsBorderColors["reviews"] }}
              >
                <DoughnutChart
                  evolution={this.state.internationalData["Evolution"]}
                  year={this.state.selectedYear["value"]}
                  colors={internationalSelectedColors}
                />
              </DataViz>

              <DataViz
                anchorId="reviews-country"
                title={this.props.t("reviews")}
                style={{ borderLeft: statsBorderColors["reviews"] }}
              >
                <DoughnutChart
            ...
            <div className="row">
              <DataViz
                anchorId="monthly-evolution-reviews-wo-france"
                title={this.props.t("monthly-wo-france")}
                style={{ borderLeft: statsBorderColors["monthly"] }}
              >
                {this.state.data["Monthly"] ? (
                  <MonthChart
                    height={250}
                    width={50}
                    evolution={this.state.internationalData["Monthly"]}
                    var="Reviews"
                    colors={internationalSelectedColors}
                  />
                ) : (
              ...
              <DataViz
                anchorId="monthly-evolution-reviews"
            ...
            <div className="row">
              <DataViz
                anchorId="yearly-evolution-reviews-wo-france"
                title={this.props.t("yearly-wo-france")}
                style={{ borderLeft: statsBorderColors["yearly"] }}
              >
                <YearChart
                  height={250}
                  width={50}
              ...
              <DataViz
                anchorId="yearly-evolution"
                title={this.props.t("yearly")}
                style={{ borderLeft: statsBorderColors["yearly"] }}
              >
                <YearChart
                  height={250}
...
export default compose(
  withTranslation(["international", "stats", "filter"]),
  connect(state => state)
)(International);