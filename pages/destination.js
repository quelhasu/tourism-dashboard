...
import {
  nationalSelectedColors,
  townshipSelectedColors,
  statsColors,
  statsBorderColors,
  departmentsSelectedColors,
  boroughSelectedColors,
  internationalSelectedColors,
  touristicColors
} from "../utils/colors";
...
class Destination extends React.Component {
  scope = [
    ...
    },
    {
      ...
      geoJSON: "https://data.dvrc.fr/api/getGeoJSONbycodetouriIDdata.php",
      name: "nom_touri"
    },
  ...
  static async getInitialProps({ req, store, isServer, pathname, query }) {
    try {
      ...
      const response = await axios.get(
        `${state.api}/${state.study}/destination/${year}/${req.params.from}/${
          req.params.groupby
        }/annual?limitareas=${limitareas}`
      );
  ...
  async componentDidMount() {
    try {
      const monthRes = await this.axiosProgress(
        `${this.props.api}/${this.props.study}/destination/${this.props.year}/${
          this.props.from
        }/${
          this.props.groupby
        }/monthly?countries=${this.selected.topCountries
          .map(el => el.value)
      ...
      const geoJSON = await axios.get(
        this.scope.find(el => el.key == this.props.groupby).geoJSON
      );
      const centralRes = await axios.get(
        `${this.props.api}/${this.props.study}/destination/${this.props.year}/${
          ...
          this.props.groupby
        }/centrality?countries=${this.selected.topCountries
          .map(el => el.value)
          .join()}&areas=${this.selected.topAreas
          .map(el => encodeURIComponent(el.value))
          .join()}`
  ...
  handleSubmit = async event => {
    ...
    try {
      ...
      const link = `${this.props.api}/${this.props.study}/destination/${
        this.state.selectedYear.value
      }/${this.props.from}/${
        this.props.groupby
      }/annual?countries=${this.selected.topCountries
        .map(el => el.value)
        .join()}&areas=${this.selected.topAreas
        .map(el => encodeURIComponent(el.value))
        .join()}&ages=${this.selected.topAges.value || "-"}`.replace(
        /\s\s+/g,
        " "
      );
      ...
      const geoJSON = scope.geoJSON
        ? await axios.get(scope.geoJSON)
        : { data: null };
      const topAreas = await axios.get(
        `${this.props.api}/${this.props.study}/destination/${
          this.state.selectedYear.value
        }/${this.props.from}/${this.props.groupby}/info/areas`
      );
            ...
            <form onSubmit={this.handleSubmit.bind(this)}>
              <div className="row">
                <label className="col-md-2">
                  <u>{this.props.t("filter:scope")}:</u>
                </label>
              </div>
              <div className="form-group row">
                <div className="col-md-4">
                  <div className="form-group row">
                    <label className="col-md-2 col-form-label text-muted">
                      {this.props.t("filter:from")}
                    </label>
                    <Nav
                      variant="pills"
                      defaultActiveKey={this.state.from}
                      onSelect={selectedKey =>
                        this.setState({ from: selectedKey, modifyScope: true })
                      }
                    >
                      {this.scope
                ...
                <div className="col-md-6">
                  <div className="form-group row">
                    <label className="col-md-2 col-form-label text-muted">
                      {this.props.t("filter:group-by")}
                    </label>
                    <Nav
                      variant="pills"
                      defaultActiveKey={this.state.groupby}
                      onSelect={selectedKey =>
                        this.setState({
                          groupby: selectedKey,
                          modifyScope: true
                ...
                {this.state.modifyScope ? (
                  this.state.from < this.state.groupby ? (
                    <div className="col-auto ml-auto">
                      <Link
                        href={`/destination/${this.state.selectedYear.value}/${
                          this.state.from
                        }/${this.state.groupby}`}
                      >
                        <a className="btn btn-outline-primary">
                          {this.props.t("filter:update-scope")}
                        </a>
                      </Link>
                    </div>
                  ) : (
        ...
        <div className="col">
          ...
          <div className="row">
            <DataViz
              ...
              title={this.props.t("ingoing-outgoing")}
            ...
            >
              <GoingChart
                ...
                year={this.state.selectedYear["value"]}
                colors={
                  this.scope.find(el => el.key == this.props.groupby).colors
                }
              />
            </DataViz>
          </div>
                ...
                >
                  {this.state.data["Monthly"] ? (
                    <MonthChart
                      ...
                      var="Ingoing"
                      colors={
                        this.scope.find(el => el.key == this.props.groupby)
                          .colors
                      }
                    />
                  ) : (
          ...
          <div className="row">
            ...
            <DataViz
              ...
              title={this.props.t("centrality-evolution")}
              second="(PageRank Y / Y-2)"
              style={{ borderLeft: statsBorderColors["central"] }}
            >
              {this.state.data["Centrality"] ? (
                <YearChartDot
...
export default compose(
  withTranslation(["destination", "stats", "filter"]),
  connect(state => state)
)(Destination);
